...
> (1) تنفيذ في محرك الواجهة `public/js/engine.js`، (2) حارس مقابل في قاعدة البيانات `db/schema.sql`، (3) اختبار آلي يثبتها.
...
| محرك قواعد العمل (بدون DOM، قابل للاختبار في Node) | `public/js/engine.js` | ✅ |
...
| تطبيق الواجهة (8 شاشات + المستخدمون، Dark Emerald RTL) | `public/` | ✅ |
| سيرفر أونلاين: حسابات دخول وأدوار، حالة مشتركة بإصدارات (Optimistic Concurrency)، تحرير آلي للحجوزات المنتهية | `server.js`, `lib/store.js` | ✅ منشور كخدمة `umrah-erp` في `render.yaml` |

**نموذج التشغيل الحالي (v4 — بصراحة):**
- **مستند لكل شركة** (`company_state`) يحتوي البيانات الأساسية + المحاسبة + **رحلات متعددة**. الرحلة المختارة "تُركّب" (`Model.mountTrip`) فتظهر مصفوفاتها (`bookings`, `rooms`, `beds`…) على كائن الشركة فيعمل محرك الرحلة كما هو.
- **حفظ بإصدارات** (Optimistic Concurrency): كل حفظ يرسل `baseVersion` ويرفض عند التعارض (409)، والنسخة السابقة تُحفظ في `state_versions` (آخر 60 + النقاط المسماة) مع نسخة يومية كاملة على القرص.
- **حوكمة السيرفر** (`lib/governance.js`) تقارن كل حفظ بالسابق: القيود append-only، السندات لا تُحذف والمعتمد لا يُعدل، الاعتماد والترحيل للمحاسب/المدير، زيادة محفظة وكيل أو تعديل مسدد/سعر/خصم حجز ممنوعة لغير المخوّلين، وبيانات الشركة والضرائب والخزائن للمدير/المحاسب.
- **البوابات** (مندوب، مشرف، مندوب تسكين) لا تستلم مستند الشركة أبداً؛ السيرفر يحسب لها عرضاً مفلتراً وينفذ حجوزاتها ودفعاتها بنفس دوال النموذج (`Model.createBooking`, `Model.createVoucher`).
- **الحدود:** مستخدم داخلي مخوّل بالحفظ يمكنه تقنياً تعديل بيانات تشغيلية غير محمية بقاعدة حوكمة (مثل بيانات مسافر)؛ كل حفظ مسجل باسمه ويمكن الرجوع عنه. نموذج PostgreSQL في `db/` هو هدف الإنتاج التالي حيث تصبح كل القواعد على مستوى قاعدة البيانات.

### المحاسبة (`public/js/accounting.js`)
- **شجرة حسابات** 4 مستويات (أصول 1 · خصوم 2 · حقوق ملكية 3 · إيرادات 4 · مصروفات 5)، القيد على الحسابات الفرعية فقط، العملة الأساسية جنيه والعملات الأخرى بسعر الصرف الفعلي للسند.
- **الدورة المستندية:** سند (قبض RV · صرف PV · مصروف EXP · تحويل TR · فاتورة مورد BILL · قيد يومية JV) ← مرفقات ← بانتظار الاعتماد ← اعتماد = ترحيل قيد تلقائي ← الإلغاء بقيد عكسي فقط.
- **الترحيل التلقائي:** إيراد الحجز يُرحّل عند تحوله لعربون/مؤكد (مع فصل ضريبة القيمة المضافة إن فُعّلت) ويُعكس عند الإلغاء — بطريقة الفرق (idempotent). عمولات الوسطاء والعمولات التشجيعية تُرحّل كالتزام للوكيل. سداد المورد بالريال يُحمّل بسعر الرحلة المرجعي ويُرحّل فرق العملة لحساب أرباح/خسائر فروق العملة.
- **دفاتر الأطراف:** كل سطر قيد يحمل الطرف (عميل/وكيل/مورد/موظف) فتخرج كشوف الحساب من القيود مباشرة.
- **التقارير:** ميزان المراجعة، دفتر الأستاذ (بحساب + طرف)، قائمة الدخل، المركز المالي، أرصدة الأطراف — بفلترة التاريخ والرحلة (مركز التكلفة) والفرع.
...
```bash
# السيرفر الأونلاين (حسابات + بيانات مشتركة)
cd umrah-erp && npm install && npm start      # http://localhost:3002
# أو عرض لمستخدم واحد بدون سيرفر: افتح umrah-erp/public/index.html

# اختبارات المحرك (20)
cd umrah-erp && npm test
...
|---|---|---|
| P0 | استبدال مستند الحالة الواحد بطبقة API (أوامر: حجز، تسكين، سداد…) فوق `db/schema.sql` | كل endpoint = معاملة واحدة؛ ترجمة أكواد الأخطاء (`GENDER_MIX_FORBIDDEN`، `CREDIT_LOCK`…) إلى رسائل عربية؛ المحرك الحالي يبقى للتحقق المسبق في الواجهة فقط |
...
| P1 | Outbox للواتساب: جدول `notifications_outbox` + عامل يرسل عبر WhatsApp Cloud API ويستقبل Webhooks التسليم | القوالب الخمسة جاهزة في `public/js/pages-ops.js` (`WA`) |
