...
**نموذج التشغيل الحالي (بصراحة):**
- النسخة الأونلاين تحفظ حالة المنظومة كلها كمستند JSON واحد في SQLite (`app_state`) برقم إصدار. كل حفظ يرسل `baseVersion`؛ لو غيّر مستخدم آخر البيانات قبلها يُرفض الحفظ (409) ويحمّل العميل آخر نسخة — لا يُكتب فوق شغل أحد أبداً. العملاء يسحبون التغييرات كل 4 ثوانٍ.
- الأدوار مربوطة بحساب الدخول (المالك/مدير = 7%، رئيس قسم = 3%، سيلز/عمليات = 0%)، وإدارة المستخدمين محمية على السيرفر للمالك فقط. **لكن** قواعد العمل نفسها (الخصم، منع الاختلاط…) تُنفّذ في المتصفح؛ مستخدم مسجّل ومتمكن تقنياً يستطيع تجاوزها بطلب API يدوي. نقل التنفيذ للسيرفر (§9) يغلق ذلك.
- نموذج PostgreSQL في `db/` هو هدف الإنتاج التالي حيث تصبح القواعد مفروضة على مستوى قاعدة البيانات.
